...
public class ApnComparator {
        ...
        for (ExcelApnParser.ApnExcel excel : apnsExcel) {
            ...
            if (excel.apn != null) {
                ...
                for (XmlApnParser.ApnEntry xml : candidatosXml) {
                    if (asignados.contains(xml))
                        continue;

                    // Imprimimos la evaluación del paso 1
                    boolean apnCoincide = xml.apn != null && xml.apn.equalsIgnoreCase(excel.apn);
                    String nombreXml = xml.atributos.getOrDefault("carrier", xml.atributos.getOrDefault("name", ""))
                            .trim();
    ...
    /**
     * Compara los atributos del APN de Excel con los del XML y rellena la lista de
     * diferencias.
     */
